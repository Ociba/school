<div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover">
            ...
            <tbody>
               @foreach ($all_classes as $i=>$class)
                <tr>
                    <td> {{$all_classes->firstItem() + $i}} </td>
                    <td class="text-wrap"> {{$class->class}} </td>
                    <td style="text-transform: capitalize"> {{$class->level}} </td>
                    <td>
       ...
       <div class="d-flex align-classs-center">
            <h5 class="card-title text-lowercase">Showing {{$all_classes->firstItem()}} to {{$all_classes->lastItem()}} out of {{$all_classes->total()}} items</h5>
            <div class="ms-auto">
            {{$all_classes->links()}}
            </div>
        </div>
